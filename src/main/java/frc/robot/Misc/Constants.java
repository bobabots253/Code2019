package frc.robot.Misc;

public class Constants {

    public static final int kPCM_ID = 0; // PCM ID

    /* Driver Settings */
    public static final double kJoystickDeadband = 0.03; // Number between 0 and 1 representing how much of joystick is "dead" zone
    public static final double kTriggerDeadband = 0.05; // Number between 0 and 1 representing how much of trigger is "dead" zone

    public static final double kDriveExpScale = 1; // What power to put overall control to, (makes small inputs smaller for better control)
    public static final double kTurnSens = 1; // Maximum normal turning rate (in percent of max) to allow robot to turn to, b/t 0 and 1

    public static final double kTurnInPlaceExpScale = 1; // What power to put turn-in-place turning to (makes small inputs smaller for better control)
    public static final double kTurnInPlaceSens = 1; // Maximum turn-in-place rate (in percent of max) to allow robot to turn to, b/t 0 and 1

    /* PID Loops */
    public static final double kF = 0; // Minimum value (in percent max speed) to move the robot, b/t 0 and 1

    // Angular velocity PID gains
    public static final double kP_Gyro = 0;
    public static final double kI_Gyro = 0;
    public static final double kD_Gyro = 0;
    public static final double kTurnScaleGyro = 20; // Sets maximum turn rate (in degrees) when using angular velocity control

    // Vision PID gains
    public static final double kP_AimVision = 0;
    public static final double kI_AimVision = 0;
    public static final double kD_AimVision = 0;

    public static final double kP_DistVision = 0;
    public static final double kI_DistVision = 0;
    public static final double kD_DistVision = 0;

    /* Drivetrain Motor IDs */
    public static final int leftMotorA = 1;
    public static final int leftMotorB = 2;
    public static final int leftMotorC = 3;

    public static final int rightMotorA = 4;
    public static final int rightMotorB = 6;
    public static final int rightMotorC = 7;

    /* Shooter Motor IDs */
    public static final int kUpperStage = 8; // ID of upper stage motor
    public static final int kLowerStage = 7; // ID of lower stage motor

    /* Hatch Solenoid IDs */
    public static final int kEjectForward = 7; // Number between 0 and 7
    public static final int kEjectReverse = 4; // Number between 0 and 7

    public static final int kRetainerForward = 1; // Number between 0 and 7
    public static final int kRetainerReverse = 3; // Number between 0 and 7


}